    ...
    static member Create
        ...
        if List.isEmpty jumpers then
            ...
            result {
                ...
                let gateState = { Starting = startingGate; CurrentJury = startingGate; CoachChange = None }
                return
                    { Id = id
                      Settings = settings
                      Hill = hill
        ...
        let execute roundIndex =
            ...
            let jr =
                JumpResultCreator.createFisJumpResult
                    jumpResultId jump jumper this.Hill roundIndex gs.CurrentReal gs.CoachChange gs.Starting
            ...
            result {
                ...
                if not startlistAfter.RoundIsFinished then
                    let updated = { this with Results = resultsAfter; Startlist = startlistAfter; Status = baseStatus }
                    return updated.ClearCoachChange()
                elif this.IsLastRound roundIndex then
                    let updated = { this with Results = resultsAfter; Startlist = startlistAfter; Status = Status.Ended }
                    return updated.ClearCoachChange()
                else
    ...
    member private this.BuildNextRoundStartlist(nextRound: RoundIndex, currentResults: Results) =
        ...
        let applyLimit =
            ...
            | RoundLimit.Exact (RoundLimitValue n) ->
                fun lst ->
                    if lst.Length <= n then lst else
                    let topN, rest = lst |> List.splitAt n
                    match topN |> List.tryLast with
                    | None -> lst
                    | Some (_, cutPts) ->
                        let tied = (topN @ rest) |> List.filter (fun (_, pts) -> pts = cutPts) |> List.map fst
                        ...
                        occupied @ ((topN @ rest) |> List.filter (fun (jid, pts) -> pts = cutPts && winners.Contains jid))
