...
module Startlist =
    ...
    module Bib =
        let create value = if value >= 1 then Some (Bib value) else None
        let value (Bib v) = v
...
with
        ...
        if List.isEmpty jumperIds then
            ...
            if not duplicatedJumperIds.IsEmpty then
                ...
                | Ok entries ->
                    let bibLookup =
                        entries
                        |> List.map (fun e -> e.JumperId, e.Bib)
                        |> Map.ofList

                    Ok { Remaining = entries; Done = []; BibOfMap = bibLookup }

    static member CreateWithBibs (assigned: (JumperId * Startlist.Bib) list)
        : Result<Startlist, Startlist.Error> =
        if List.isEmpty assigned then
            ...
            if not duplicatedJumperIds.IsEmpty then
                ...
                if not duplicatedBibValues.IsEmpty then
                    ...
                    let entries =
                        assigned
                        |> List.map (fun (jumperId, bib) -> { Startlist.Entry.JumperId = jumperId; Bib = bib }: Startlist.Entry)
                        |> List.sortBy (fun e -> Startlist.Bib.value e.Bib)

                    let bibLookup = assigned |> Map.ofList
                    Ok { Remaining = entries; Done = []; BibOfMap = bibLookup }

    member this.NextEntry : Startlist.Entry option =
        this.Remaining |> List.tryHead
    ...
    member this.MarkJumpDone (jumperId: JumperId) : Result<Startlist, Startlist.Error> =
        ...
        | next :: tail when next.JumperId = jumperId ->
            Ok { this with Remaining = tail; Done = next :: this.Done }
        | next :: _ ->
            Error (Startlist.Error.JumperNotNext (jumperId, next))

    member this.RoundIsFinished : bool =
    ...
    member this.BibOf (jumperId: JumperId) : Startlist.Bib option =
        this.BibOfMap |> Map.tryFind jumperId

    static member WithOrder (previous: Startlist) (order: JumperId list)
        : Result<Startlist, Startlist.Error> =
        ...
        if order.Length <> knownJumperIds.Count then
            ...
            let firstUnknown =
                order |> List.tryFind (fun jid -> not (Set.contains jid knownJumperIds)) |> Option.get
            ...
            let entries =
                ...
                |> List.map (fun jid ->
                    ...
                    { Startlist.Entry.JumperId = jid; Bib = bib } : Startlist.Entry)

            Ok { Remaining = entries; Done = []; BibOfMap = previous.BibOfMap }