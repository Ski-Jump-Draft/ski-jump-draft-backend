...
module Hill =
    let hillTypeOf (hill: Hill) = HillType.fromHS hill.HSPoint

    let pointsPerMeter (hill: Hill) : HillPointsForMeter = HillPointsForMeter.fromK hill.KPoint

    let pointsPerKPoint (hill: Hill) : HillPointsForKPoint =
        hill |> hillTypeOf |> HillPointsForKPoint.fromHillType
